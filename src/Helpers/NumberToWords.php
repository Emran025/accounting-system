...
class NumberToWords
{
    ...
    public static function convert($number, $locale = 'ar', $currency = null, $precision = 2)
    {
        ...
        $defaults = [
            'ar' => [
                ...
                // Include singular_acc for subunits so numbers 11-99 use singular accusative (e.g., "خمسون هللة")
     ...
     * @param string $intPart integer part as string
     * @param array $unit array with keys 'singular','dual','plural','singular_acc'
     * @return string
     */
    /**
     * Return arabic currency form following tamyeez rules described.
     ...
     *
     * This method is used for both major units and subunits (e.g., هللة) so the
     * same tamyeez decisions apply to fractional parts like 50 -> "هللة" (singular accusative).
     *
     * @param string $intPart integer part as string
     * @param array $unit ['singular','dual','plural','singular_acc'] (subunits should include singular_acc)
     * @return string
     */
    protected static function arabicCurrencyForm(string $intPart, array $unit): string
    {
        $n = (int)$intPart;
        if ($n === 0) {
            return $unit['singular']; // "صفر ريال"
        }
        // If divisible by 100 -> singular (e.g., 2500 -> ألفان وخمسمائة ريال)
        if ($n % 100 === 0) {
    ...
    /**
     * Extract sign and absolute numeric string
     * @param string $numStr
     * @return array [sign, absoluteString]
     */
    ...
    /**
     * Split integer and fractional part (frac padded as needed)
     * @param string $absStr
     * @param int $precision
     * @return array [intPart, fracPart]
     */
    protected static function splitDecimals(string $absStr, int $precision): array
    {
        ...
        // pad or trim fractional to more than precision, trimming will be handled in rounding
        return [$i === '' ? '0' : $i, rtrim($f, '0')];
    }

    /**
     * Round using string math to the given precision and return normalized int and frac (strings)
     * @param string $intPart
     * @param string $fracPart (unrounded)
     * @param int $precision
     * @return array [intPart, fracPart]
     */
    protected static function roundStringNumber(string $intPart, string $fracPart, int $precision): array
    {
        // Ensure fracPart has at least precision+1 digits for rounding decision
        ...
        if ($roundDigit >= 5) {
            // Add 1 to target using string math
            $carry = 1;
            $t = $target;
            $len = strlen($t);
            for ($i = $len - 1; $i >= 0; $i--) {
            ...
            if ($carry === 1) {
                // fractional rolled over to next integer
                $intPart = (string)((int)$intPart + 1);
                $t = str_repeat('0', $precision);
            }
            $target = $t;
        } else {
            // no rounding up, just pad/truncate
