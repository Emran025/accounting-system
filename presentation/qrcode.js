 ...
 * Modified version for supermarket system
 */
var qrcode = (function () {
  var PAD0 = 0xec;
  var PAD1 = 0x11;
  ...
  var setupTypeInfo = function (test, maskPattern) {
    var data =
      (QRUtil.getErrorCorrectionLevel(_errorCorrectionLevel) << 3) |
      maskPattern;
  ...
  QRRSBlock.RS_BLOCK_TABLE = [
    ...
    [2, 121, 97],
    [4, 121, 97],
    [4, 121, 97], // Simplified for type 1-8
  ];
  QRRSBlock.getRSBlocks = function (typeNumber, errorCorrectionLevel) {
    var rsBlock = null;
    switch (errorCorrectionLevel) {
      case "L":
        rsBlock = QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 0];
        break;
      case "M":
        rsBlock = QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 1];
        break;
      case "Q":
        rsBlock = QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 2];
        break;
      case "H":
        rsBlock = QRRSBlock.RS_BLOCK_TABLE[(typeNumber - 1) * 4 + 3];
        break;
    }
    var list = [];
  ...
  QRBitBuffer.prototype = {
    get: function (index) {
      return (
        ((this.buffer[Math.floor(index / 8)] >>> (7 - (index % 8))) & 1) == 1
      );
    },
  ...
  var QRUtil = {
    ...
    getErrorCorrectionLevel: function (level) {
      switch (level) {
        case "L":
          return 1;
        case "M":
...
// Global Helper to easily generate Base64 QR Image
window.createBase64QR = function (text, cellSize) {
  try {
    var qr = qrcode(0, "M");
    qr.addData(text);
    qr.make();
