...
Linux private data is owned by the `accore` system account; systemd uses `NoNewPrivileges`, `PrivateTmp`, `ProtectSystem=strict`, `ProtectHome`, a restrictive `UMask`, and explicit writable paths. On Windows, first-claim and service startup harden the private data before any worker starts. Reconciliation and ownership transition of an existing configuration change only protected metadata and SCM state; they never re-traverse a live data tree while its restore-validation worker can create or delete files. ACL hardening remains fail-closed for the private root and every surviving entry; its narrow descendant re-check handles only an entry that has demonstrably disappeared. The macOS plist is written beneath `/Library/LaunchDaemons`, owned by `root:wheel`, and mode `0600`. The agent runs in the foreground because launchd, like systemd, is the process supervisor rather than the application itself.[1] [2]
...
| ------------------- | ------------------------------- | ------------------------------------------------------------------ |
| Linux x64           | `frankenphp-linux-x86_64`       | `53fdfab33d7464ccdc9a2fa9cc39de6811c8da8ff5e592dd800f233eb5313e5d` |
| Windows x64         | `frankenphp-windows-x86_64.zip` | `edec8d3c43508f98b498af911f47aa93ebc51f7e46f1d26a9d41adb7ccbaa828` |
| macOS Apple Silicon | `frankenphp-mac-arm64`          | `90e2f86a23154f48738453c2235f9289ea4e34e0b67f306336cb2a8d93fe787a` |
| macOS Intel         | `frankenphp-mac-x86_64`         | `ec0adfa9ad7bbe6c11d9a70eb6f357340d9d63b720e2c971781d1fcd0e183acb` |
...
The MariaDB Unix bootstrap is relocatable. The Agent invokes `mariadb-install-db` with `--no-defaults` first, an explicit runtime `--basedir`, and `--skip-test-db`; it retains only MariaDB's `share/mariadb_test_db.sql` **bootstrap template**, which the upstream script validates before honoring `--skip-test-db`, while it still removes the test suite, test clients, benchmark payload, and the actual `test` database. The Agent creates a private socket inside its data root. The first provisioning operation authenticates only through that socket as the normal root account with no password, sets the randomized root credentials, provisions a TCP-local root account, removes anonymous/test defaults, and writes the durable credential marker. Later provisioning, backup, and restore-validation operations use the authenticated loopback TCP account. This avoids both build-host defaults and an unauthenticated network bootstrap.[13]

The macOS PKG builder discovers the Agent sidecar relative to the `.app` root, then converts it once to a `Contents`-relative path for both the PKG file-list contract and postinstall. This prevents a duplicated `Contents/Contents/MacOS` path while retaining an explicit check that the shipped sidecar is executable.

After the macOS runtime is staged, the build removes FrankenPHP's residual `/usr/local/lib` search path **only after proving that the executable has no `@rpath` dependency that could use it**, then executes the non-interactive binary contract `frankenphp --version`. It identifies **every Mach-O file** in the runtime and later in the completed Tauri application, runs `otool -L`, inspects each `LC_RPATH`, and resolves every actual `LC_LOAD_DYLIB` `@loader_path`, `@executable_path`, and `@rpath` reference to an existing file contained by the payload. A dylib's matching `LC_ID_DYLIB` is validated separately as an advertised, portable identity rather than treated as a self-dependency; this does not exempt any other load command. It rejects absolute build-machine paths and any relative reference that escapes or fails to resolve inside the payload. To avoid serial debugging, verification scans every Mach-O candidate and reports all failures in one diagnostic rather than stopping after the first external dependency. The CI then builds PKG payloads with recommended system ownership, verifies their file lists, installs each macOS PKG on the runner, checks the root-owned agent and LaunchDaemon, waits for public `ready` status, repeats installation to prove that the protected configuration and data remain, and checks the private persistence sentinel through `sudo` so the non-privileged CI user does not misread intentionally restricted traversal as missing data. It finally cleans the runner. The build tools may be present on CI, but their libraries must never become an undeclared end-user dependency.[8]
...
[13] [MariaDB — mariadb-install-db](https://mariadb.com/docs/server/clients-and-utilities/deployment-tools/mariadb-install-db)
